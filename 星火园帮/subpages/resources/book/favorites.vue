<template>
  <view class="favorites-container">
    ...
    <view class="favorites-list" v-if="favoritesList.length > 0">
      ...
      >
        ...
        <view class="item-cover-container">
          <image
            class="item-cover"
            :src="(item.book && item.book.coverImage) || '/static/images/default-book.png'"
            mode="aspectFill"
          />
          <view class="item-badge" v-if="item.book && item.book.isNew">NEW</view>
        </view>
        <view class="item-info">
          <view class="item-title">{{ (item.book && item.book.title) || "未知图书" }}</view>
          <view class="item-author" v-if="item.book && item.book.author">
            作者：{{ item.book.author }}
          </view>
          <view class="item-publisher" v-if="item.book && item.book.publisher">
            出版社：{{ item.book.publisher }}
          </view>
          <view class="item-stats">
            <text class="stat-item"
              >{{ (item.book && item.book.downloads) || 0 }}次下载</text
            >
            <text class="stat-item"
              >{{ (item.book && item.book.views) || 0 }}次浏览</text
            >
          </view>
    ...
    <view class="empty-state" v-else-if="!isLoading">
      ...
      <text class="empty-text">暂无收藏的图书</text>
      <view class="empty-actions">
        <button class="empty-btn" @click="goToBookList">去浏览</button>
      </view>
    </view>
...
export default {
  ...
  methods: {
    ...
    doRemoveFavorite(itemId) {
      ...
      favorites = favorites.filter((id) => id !== item.bookId);
      ...
      uni.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            ...
            // 获取要删除的图书ID
            const bookIdsToRemove = this.selectedItems
              .map((itemId) => {
                const item = this.favoritesList.find((i) => i.id === itemId);
                return item ? item.bookId : null;
              })
              .filter((id) => id !== null);

            // 从本地存储中删除
            favorites = favorites.filter((id) => !bookIdsToRemove.includes(id));
